    ...
    return `
      <article class="target-card">
        ...
        <dl>
          ...
          <dt>Legacy contribution</dt><dd>${formatNumber(target.legacyContribution, 1)}</dd>
  ...
  els.body.innerHTML = rows.map((row, index) => {
    ...
    return `
      <tr>
        ...
        <td><span class="target-name">${escapeHtml(row.topTarget)}</span><div class="target-meta">${formatNumber(row.topTargetPoints, 1)} pts · score ×${formatNumber(row.topTargetScoreMultiplier, 2)}</div><div class="target-meta"><span class="pill ${topClass}">${topLabel}</span></div></td>
